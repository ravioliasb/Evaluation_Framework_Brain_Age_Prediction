...
#     shuff_data = pd.read_csv(root_dir + "shuff_files_pediatric.csv")
...
def load_data(root_dir, batch):
    # Load the specific CSVs created from the pickle splits
    train_data = pd.read_csv(os.path.join(root_dir, "pediatric_train.csv"))
    val_data = pd.read_csv(os.path.join(root_dir, "pediatric_val.csv"))

    # Slicing for quick testing: take only the first 10 rows
    # train_data = train_data.head(10)
    # val_data = val_data.head(10)

    print(f"Total train: {len(train_data)}, Total val: {len(val_data)}")

    # Helper to convert DF to list of dicts
    def get_filenames(df):
        return [
            {"img": r['imgs'], "seg_label": r['seg'], "age_label": r['age'], 'brain_mask': r['mask']} 
            for _, r in df.iterrows()
        ]

    filenames_train = get_filenames(train_data)
    filenames_val = get_filenames(val_data)

    ds_train = monai.data.Dataset(filenames_train, train_transforms)
    train_loader = DataLoader(
        ds_train, batch_size=batch, shuffle=True, 
        num_workers=2, pin_memory=True, collate_fn=pad_list_data_collate
    )

    ds_val = monai.data.Dataset(filenames_val, val_transforms)
    val_loader = DataLoader(
        ds_val, batch_size=batch, shuffle=False, # Usually don't need to shuffle validation
        num_workers=1, pin_memory=True, collate_fn=pad_list_data_collate
    )
